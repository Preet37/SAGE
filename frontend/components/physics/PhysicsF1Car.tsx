...
export default function PhysicsF1Car({ params = {}, simConfig }: any) {
  const wheelFL = useRef<any>(null);
  const wheelFR = useRef<any>(null);
  const wheelRL = useRef<any>(null);
  const wheelRR = useRef<any>(null);
  const rearWingRef = useRef<any>(null);
  const frontWingRef = useRef<any>(null);
  const exhaustGlowRef = useRef<any>(null);
