...
export default function VoiceOnlyMode({ onTranscript, onClose, lessonTitle, messages, isStreaming, language = 'en' }: Props) {
  ...
  const pulse = voiceState === 'listening'
    ...
    : voiceState === 'speaking'
    ? 'animate-pulse bg-blue-500/20 border-blue-400'
    : voiceState === 'thinking'
  ...
  return (
    <div
      className="fixed inset-0 z-50 bg-gray-950 flex flex-col items-center justify-center gap-8"
      aria-live="polite"
      aria-label="Voice-only mode"
    >
      <button
        onClick={() => { window.speechSynthesis?.cancel(); onClose(); }}
        className="absolute top-4 right-4 text-gray-400 hover:text-white text-sm px-3 py-1.5 rounded-lg border border-white/10 hover:border-white/20 transition-all"
      >
      ...
      <div className="text-center">
        <div className="text-4xl font-black tracking-tight text-white">
          S<span className="text-blue-400">AGE</span>
        </div>
        <div className="text-gray-400 text-sm mt-1">{lessonTitle}</div>
      </div>
      ...
      <div className="text-gray-300 text-sm font-medium h-5">{label}</div>

      {transcript && (
        <div className="max-w-sm text-center text-gray-400 text-xs italic px-4">{transcript}</div>
      )}

      {(() => {
        const last = messages.filter((m) => m.role === 'assistant').at(-1);
        return last?.content ? (
          <div className="max-w-md text-center text-gray-200 text-sm leading-relaxed px-6 py-4 bg-white/5 rounded-2xl border border-white/10">
            {last.content
              .replace(/[*_`#~]/g, '')
              .replace(/<[^>]+>/g, '')
              .slice(0, 300)}
