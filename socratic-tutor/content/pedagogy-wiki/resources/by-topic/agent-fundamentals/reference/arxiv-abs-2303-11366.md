...
1 Introduction
Recent works such as ReAct Yao et al., ([2023](#bib.bib30)), SayCan Ahn et al., ([2022](#bib.bib1)),
Toolformer Schick et al., ([2023](#bib.bib22)), HuggingGPT (Shen et al.,, [2023](#bib.bib23)),
generative agents (Park et al.,, [2023](#bib.bib19)), and WebGPT (Nakano et al.,, [2021](#bib.bib17))
...
Generating useful reflective feedback is challenging since it requires a good understanding of where the model made mistakes (i.e. the credit assignment problem (Sutton and Barto,, [2018](#bib.bib25))) as well as the ability to generate a summary containing actionable insights for improvement. We explore three ways for
...
Reflexion agents improve on decision-making AlfWorld (Shridhar et al.,, [2021](#bib.bib24)) tasks over strong baseline approaches by
an absolute 22% in 12 iterative learning steps, and
on reasoning questions in HotPotQA (Yang et al.,, [2018](#bib.bib28)) by 20%, and Python programming
tasks on HumanEval (Chen et al.,, [2021](#bib.bib6)) by as much as 11%.
...
Self-Refine (Madaan et al.,, [2023](#bib.bib15)) employs an iterative framework for
...
but is limited to single-generation reasoning tasks. Pryzant et al., ([2023](#bib.bib21))
performs a similar semantic prompt-writing optimization, but is
also limited to single-generation tasks.
Paul et al., ([2023](#bib.bib20)) fine-tune critic models to provide intermediate
feedback within trajectories to improve reasoning responses.
Xie et al., ([2023](#bib.bib27)) use stochastic beam search over actions to
...
Yoran et al., ([2023](#bib.bib31)) and Nair et al., ([2023](#bib.bib16)) use decider models
to reason over several generations. Kim et al., ([2023](#bib.bib10)) use a
retry pattern over a fixed number of steps without an evaluation step.
Goodman, ([2023](#bib.bib9)) perform a qualitative evaluation step that
...
or code debugging practices. AlphaCode (Li et al.,, [2022](#bib.bib14))
evaluates a set of generations on hidden test cases. CodeT (Chen et al.,, [2022](#bib.bib5))
uses self-generated unit tests that are used to score generated function
implementations.
Self-Debugging (Chen et al.,, [2023](#bib.bib7)) employs a debugging component that is used
to improve existing implementations given feedback from a code
execution environment. CodeRL (Le et al.,, [2022](#bib.bib12)) sets the problem
...
including Chain of Thought Wei et al., ([2022](#bib.bib26)) and ReAct Yao et al., ([2023](#bib.bib30)).
...
was inspired by Brooks et al., ([2022](#bib.bib3)), who suggest a policy iteration approach using in-context learning. Details on how this
...
search-based question answering on HotPotQA (Yang et al.,, [2018](#bib.bib28)), multi-step
tasks in common household environments in AlfWorld (Shridhar et al.,, [2021](#bib.bib24)), and code
writing tasks in competition-like environments with interpreters and
compilers in HumanEval (Chen et al.,, [2021](#bib.bib6)), MBPP (Austin et al.,, [2021](#bib.bib2)),
...
(Côté et al.,, [2019](#bib.bib8)). Following Yao et al., ([2023](#bib.bib30)), we run the agent in
...
chilling a tomato in the fridge). We use ReAct (Yao et al.,, [2023](#bib.bib30)) as the
action generator as Yao et al., ([2023](#bib.bib30)) has shown success in long trajectory
...
trajectory examples as Yao et al., ([2023](#bib.bib30)) with GPT-3 for the LLM. AlfWorld
...
HotPotQA (Yang et al.,, [2018](#bib.bib28)) is a Wikipedia-based dataset with 113k
...
+ Chain-of-Thought (CoT) (Wei et al.,, [2022](#bib.bib26)) for step-by-step and
...
implement a Reflexion + ReAct (Yao et al.,, [2023](#bib.bib30)) agent that can retrieve
...
writing on MBPP (Austin et al.,, [2021](#bib.bib2)), HumanEval (Chen et al.,, [2021](#bib.bib6)),
and LeetcodeHardGym, our new dataset. MBPP and HumanEval measure function body
generation accuracy given natural language
descriptions. We use a benchmark language compiler, MultiPL-E (Cassano et al.,, [2022](#bib.bib4)),
...
October 8, 2022, which is the pre-training cutoff date of GPT-4 (OpenAI,, [2023](#bib.bib18)).
...
|---|---|---|---|
| HumanEval (PY) | 65.8 (CodeT (Chen et al.,,
|
[2022](#bib.bib5)) + Codex (Chen et al.,,
[2021](#bib.bib6)))
...
[2022](#bib.bib4)). TP: unit tests pass, solution pass; FN: unit tests fail, solution pass; FP: unit tests pass, solution fail; TN: unit tests fail, solution fail.
...
solution and lead to a false positive label on a code completion (Lam et al.,, [2020](#bib.bib11)).
...
shortcoming in an experiment on WebShop ([Yao et al., rint, ](#bib.bib29)). WebShop is a web-based
