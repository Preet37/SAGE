...
# Title: Self-Attention from Scratch (Sebastian Raschka)
# Fetched via: browser
# Date: 2026-04-08
...
Since we will need those to compute the attention scores later, let’s compute the 
ω
𝜔
...
α
𝛼
, by applying the softmax function. Additionally, 
1/
d
k
‾
...
The scaling by the square-root of 
d
k
𝑑
𝑘
...
𝑑
𝑘
? The dot product between q and k is a sum of 
d
k
𝑑
𝑘
 independent terms, each with variance about 1. That means the variance of the raw score grows linearly with 
d
k
𝑑
𝑘
...
𝑑
𝑘
, we cancel that growth and bring the variance back to about 1.
...
×
𝑑
 dimensional weight matrices so 
3×
d
′
×d
...
Consequently, each query element is now 
3×
d
q
3
...
Then, we follow the same steps as previously to compute the unscaled attention weights 
ω
𝜔
 and attention weights 
α
𝛼
, followed by the scaled-softmax computation to obtain an 
h×
d
v
ℎ
...
In the code walkthrough above, we set 
d
q
=
d
...
=
28
. Or in other words, we used the same dimensions for query and key sequences. While the value matrix 
W
v
𝑊
𝑣
...
The figure below illustrates the concept of cross-attention. If we set 
x
1
=
x
