...
# Title: 10.7. Sequence-to-Sequence Learning for Machine Translation — Dive into Deep Learning
# Fetched via: trafilatura
# Date: 2026-04-07

10.7. Sequence-to-Sequence Learning for Machine Translation[¶](#sequence-to-sequence-learning-for-machine-translation)[ ](https://studiolab.sagemaker.aws/import/github/d2l-ai/d2l-pytorch-sagemaker-studio-lab/blob/main/GettingStarted-D2L.ipynb) Open the notebook in SageMaker Studio Lab
In so-called sequence-to-sequence problems such as machine translation
(as discussed in [Section 10.5](machine-translation-and-dataset.html#sec-machine-translation)), where inputs and
outputs each consist of variable-length unaligned sequences, we
generally rely on encoder–decoder architectures
([Section 10.6](encoder-decoder.html#sec-encoder-decoder)). In this section, we will demonstrate
...
translation
([Cho et al., 2014](../chapter_references/zreferences.html#id41), [Sutskever et al., 2014](../chapter_references/zreferences.html#id273)).
...
[Section 11](../chapter_attention-mechanisms-and-transformers/index.html#chap-attention-and-transformers), we will introduce attention
...
normal language model. [Fig. 10.7.1](#fig-seq2seq) illustrates how to use
two RNNs for sequence-to-sequence learning in machine translation.
In [Fig. 10.7.1](#fig-seq2seq), the special “<eos>” token marks the end of
...
decoding time step ([Cho et al., 2014](../chapter_references/zreferences.html#id41)). In
some other designs, such as that of
Sutskever et al. ([2014](../chapter_references/zreferences.html#id273)), the final hidden state of the RNN
...
10.7.1. Teacher Forcing[¶](#teacher-forcing)
...
“.”, “<eos>” ([Fig. 10.7.1](#fig-seq2seq)).
Our implementation in [Section 10.5.3](machine-translation-and-dataset.html#subsec-loading-seq-fixed-len) prepared
...
in [Section 9.3](../chapter_recurrent-neural-networks/language-model.html#sec-language-model). An alternative approach is to feed
the predicted token from the previous time step as the current input
to the decoder.
In the following, we explain the design depicted in
[Fig. 10.7.1](#fig-seq2seq) in greater detail. We will train this model for
machine translation on the English–French dataset as introduced in
[Section 10.5](machine-translation-and-dataset.html#sec-machine-translation).
10.7.2. Encoder[¶](#encoder)
Recall that the encoder transforms an input sequence of variable length
into a fixed-shape context variable \(\mathbf{c}\) (see
[Fig. 10.7.1](#fig-seq2seq)).
...
[¶](#equation-chapter-recurrent-modern-seq2seq-0)\[\mathbf{h}_t = f(\mathbf{x}_t, \mathbf{h}_{t-1}).\]
...
[¶](#equation-chapter-recurrent-modern-seq2seq-1)\[\mathbf{c} = q(\mathbf{h}_1, \ldots, \mathbf{h}_T).\]
For example, in [Fig. 10.7.1](#fig-seq2seq), the context variable is just
...
(embed_size
). For any input token index \(i\), the embedding
...
[¶](#equation-eq-seq2seq-s-t)\[\mathbf{s}_{t^\prime} = g(y_{t^\prime-1}, \mathbf{c}, \mathbf{s}_{t^\prime-1}).\]
...
Following [Fig. 10.7.1](#fig-seq2seq), when implementing the decoder as
...
[Fig. 10.7.2](#fig-seq2seq-details).
10.7.4. Encoder–Decoder for Sequence-to-Sequence Learning[¶](#encoderdecoder-for-sequence-to-sequence-learning)
...
10.7.5. Loss Function with Masking[¶](#loss-function-with-masking)
...
optimization. Recall from [Section 10.5](machine-translation-and-dataset.html#sec-machine-translation) that the
...
[Fig. 10.7.3](#fig-seq2seq-predict). When the end-of-sequence (“<eos>”)
...
based on beam search ([Section 10.8](beam-search.html#sec-beam-search)).
...
10.7.8. Evaluation of Predicted Sequences[¶](#evaluation-of-predicted-sequences)
...
([Papineni et al., 2002](../chapter_references/zreferences.html#id206)), has been extensively used in
measuring the quality of output sequences for different applications. In
principle, for any \(n\)-gram
([Section 9.3.1.1](../chapter_recurrent-neural-networks/language-model.html#subsec-markov-models-and-n-grams)) in the predicted
...
[¶](#equation-eq-bleu)\[\exp\left(\min\left(0, 1 - \frac{\textrm{len}_{\textrm{label}}}{\textrm{len}_{\textrm{pred}}}\right)\right) \prod_{n=1}^k p_n^{1/2^n},\]
where \(k\) is the longest \(n\)-gram for matching.
Based on the definition of BLEU in [(10.7.4)](#equation-eq-bleu), whenever the
...
multiplication term in [(10.7.4)](#equation-eq-bleu) penalizes shorter predicted
