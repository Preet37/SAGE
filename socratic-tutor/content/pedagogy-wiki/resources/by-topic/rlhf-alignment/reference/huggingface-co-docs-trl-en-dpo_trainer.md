...
Quick start
This example demonstrates how to train a language model using the [DPOTrainer](/docs/trl/v1.0.0/en/bema_for_reference_model#trl.DPOTrainer) from TRL. We train a [Qwen 3 0.6B](https://huggingface.co/Qwen/Qwen3-0.6B) model on the [UltraFeedback dataset](https://huggingface.co/datasets/openbmb/UltraFeedback).
...
The [DPOTrainer](/docs/trl/v1.0.0/en/bema_for_reference_model#trl.DPOTrainer) tokenizes each input using the model’s tokenizer.
Computing the loss
...
in [DPOConfig](/docs/trl/v1.0.0/en/dpo_trainer#trl.DPOConfig)to model label-flip probability; valid values are in the range[0.0, 0.5)
."bco_pair"
[BCO](https://huggingface.co/papers/2404.04656)authors train a binary classifier whose logit serves as a reward so that the classifier maps {prompt, chosen completion} pairs to 1 and {prompt, rejected completion} pairs to 0. For unpaired data, we recommend the dedicated[experimental.bco.BCOTrainer](/docs/trl/v1.0.0/en/bco_trainer#trl.experimental.bco.BCOTrainer)."sppo_hard"
...
Some argument combinations are intentionally restricted in the current [DPOTrainer](/docs/trl/v1.0.0/en/bema_for_reference_model#trl.DPOTrainer) implementation:
use_weighting=True
...
method to the [DPOConfig](/docs/trl/v1.0.0/en/dpo_trainer#trl.DPOConfig). For example, if you want to load a model in a different precision, analogous to
...
argument to the [DPOConfig](/docs/trl/v1.0.0/en/dpo_trainer#trl.DPOConfig).
...
trainer.train()
You can also continue training your PeftModel
. For that, first load a PeftModel
outside [DPOTrainer](/docs/trl/v1.0.0/en/bema_for_reference_model#trl.DPOTrainer) and pass it directly to the trainer without the peft_config
...
The [DPOTrainer](/docs/trl/v1.0.0/en/bema_for_reference_model#trl.DPOTrainer) fully supports fine-tuning models with tool calling capabilities. In this case, each dataset example should include:
...
[DPOTrainer](/docs/trl/v1.0.0/en/bema_for_reference_model#trl.DPOTrainer) fully supports training Vision-Language Models (VLMs). To train a VLM, provide a dataset with either an image
...
[< source >](https://github.com/huggingface/trl/blob/v1.0.0/trl/trainer/dpo_trainer.py#L406)
...
or[PreTrainedModel](https://huggingface.co/docs/transformers/v5.4.0/en/main_classes/model#transformers.PreTrainedModel)orPeftModel
...
[save_pretrained](https://huggingface.co/docs/transformers/v5.4.0/en/main_classes/model#transformers.PreTrainedModel.save_pretrained), e.g.,'./my_model_directory/'
...
. - A
[PreTrainedModel](https://huggingface.co/docs/transformers/v5.4.0/en/main_classes/model#transformers.PreTrainedModel)object. Only causal language models are supported. - A
PeftModel
...
[args (](#trl.DPOTrainer.args)[DPOConfig](/docs/trl/v1.0.0/en/dpo_trainer#trl.DPOConfig), optional) — Configuration for this trainer. IfNone
...
on your model and a scheduler given by[get_linear_schedule_with_warmup](https://huggingface.co/docs/transformers/v5.4.0/en/main_classes/optimizer_schedules#transformers.get_linear_schedule_with_warmup)controlled byargs
.[peft_config (](#trl.DPOTrainer.peft_config)PeftConfig
...
This class is a wrapper around the [Trainer](https://huggingface.co/docs/transformers/v5.4.0/en/main_classes/trainer#transformers.Trainer) class and inherits all of its attributes and methods.
Example:
from trl import DPOTrainer
from datasets import load_dataset
dataset = load_dataset("trl-lib/ultrafeedback_binarized", split="train")
...
trainer.train()
train
[< source >](https://github.com/huggingface/trl/blob/v1.0.0/transformers/trainer.py#L1322)
...
.[max_length (](#trl.DPOConfig.max_length)int
orNone
...
. Possible values are"keep_end"
and"keep_start"
.[padding_free (](#trl.DPOConfig.padding_free)bool
...
.
Configuration class for the [DPOTrainer](/docs/trl/v1.0.0/en/bema_for_reference_model#trl.DPOTrainer).
