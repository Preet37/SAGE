...
1 Introduction
Scaling large language models has unlocked a new class of autonomous software agents—systems that perceive environments, reason about goals, wield tools, and take action over extended time horizons [Brown et al., [2020](#bib.bib26), Achiam et al., [2023](#bib.bib28), Touvron et al., [2023](#bib.bib27)].
...
Memory transforms a stateless LLM into a self-evolving agent [Zhang et al., [2024b](#bib.bib18)] that can (i) accumulate factual knowledge and user preferences, (ii) develop behavioral patterns grounded in prior experience, (iii) avoid repeating costly mistakes, and (iv) continuously improve through interaction.
1.2 A brief history of neural memory
The ambition to give neural networks external storage dates back over a decade.
Memory Networks [Weston et al., [2015](#bib.bib61)] and their end-to-end variant [Sukhbaatar et al., [2015](#bib.bib62)] introduced differentiable read-write access to external slots for question answering.
Neural Turing Machines [Graves et al., [2014](#bib.bib63)] and Differentiable Neural Computers [Graves et al., [2016](#bib.bib64)] pushed further, supporting both content-based and location-based addressing over a memory matrix.
Memorizing Transformers [Wu et al., [2022](#bib.bib66)] and Recurrent Memory Transformers [Bulatov et al., [2022](#bib.bib65)] later integrated explicit memory layers directly into the Transformer backbone.
A parallel thread focused on retrieval.
RAG [Lewis et al., [2020](#bib.bib1)] married a pre-trained generator with a dense document retriever, and RETRO [Borgeaud et al., [2022](#bib.bib2)] showed that pulling from a trillion-token corpus at inference time could match much larger models at a fraction of the parameter count.
...
ReAct [Yao et al., [2022](#bib.bib3)] interleaved reasoning traces with environment actions, producing an interpretable trajectory that doubles as short-horizon memory.
Reflexion [Shinn et al., [2023](#bib.bib4)] took this further by storing verbal self-critiques after task attempts—essentially giving the agent a post-mortem journal.
Then came the Generative Agents paper [Park et al., [2023](#bib.bib7)], whose simulated town of 25 characters demonstrated that a simple observation–reflection–planning loop could produce months of coherent social behavior.
Since 2023, the design space has exploded: hierarchical virtual memory inspired by operating systems [Packer et al., [2024](#bib.bib10)], ever-growing skill libraries in Minecraft [Wang et al., [2023a](#bib.bib8)], SQL databases as symbolic memory [Hu et al., [2023](#bib.bib16)], and—most recently—end-to-end learned memory management via reinforcement learning [Yu et al., [2026](#bib.bib11)].
1.3 Why another survey?
Several broad agent surveys already exist [Xi et al., [2023](#bib.bib19), Wang et al., [2024a](#bib.bib20)], and Zhang et al. [Zhang et al., [2024b](#bib.bib18)] published a memory-focused review in 2024.
However, the landscape has shifted considerably since then.
A wave of 2025–2026 contributions—Agentic Memory [Yu et al., [2026](#bib.bib11)], MemBench [Tan et al., [2025](#bib.bib23)], MemoryAgentBench [Hu et al., [2025](#bib.bib24)], MemoryArena [He et al., [2026](#bib.bib25)]—has introduced learned memory control, richer evaluation dimensions, and agentic benchmarks that tightly couple memory with action.
...
At each discrete step , an agent receives input —a user message, a sensor reading, or a tool return value—and must produce an action . Between these two events, it consults its accumulated memory. We write:
| (1) | ||||
| (2) |
where is the policy (typically a prompted or partially fine-tuned LLM), reads from memory, writes to and manages memory, encodes active goals, is environment feedback, and is any reward-like signal.
Two aspects deserve emphasis. First, is not a simple append operation. In a well-designed system it summarizes, deduplicates, scores priority, resolves contradictions, and—when appropriate—deletes. Second, and form a feedback loop: the agent’s decisions determine what gets written, and what is written shapes future decisions. This recursive dependence is what makes memory both powerful and brittle—one bad write can pollute the store for many steps downstream.
2.2 Connection to POMDPs
Cast formally, the setup above is a partially observable Markov decision process. Memory plays the role of the agent’s belief state: an internal summary of history that stands in for the unobservable true state of the world. Classical POMDP solvers update beliefs via Bayesian filtering; LLM agents do something analogous—albeit messier—through natural language compression, vector indexing, or structured storage.
...
In the Generative Agents experiment [Park et al., [2023](#bib.bib7)], removing the reflection component caused agent behavior to degenerate from coherent multi-day planning to repetitive, context-free responses within 48 simulated hours.
Voyager [Wang et al., [2023a](#bib.bib8)] without its skill library lost 15.3 in tech-tree milestone speed—the skill library was the performance.
And in MemoryArena [He et al., [2026](#bib.bib25)], swapping an active memory agent for a long-context-only baseline dropped task completion from over 80% to roughly 45% on interdependent multi-session tasks.
...
Cognitive scientists have long distinguished multiple memory systems in the human brain [Atkinson and Shiffrin, [1968](#bib.bib30), Tulving, [1972](#bib.bib31), Baddeley, [2000](#bib.bib32), Squire, [2004](#bib.bib33)].
...
Baddeley’s central executive plus buffer model [Baddeley, [2000](#bib.bib32)] maps neatly: the LLM is the executive, the context window is the buffer, and both share the same bottleneck—limited capacity.
Episodic memory.
Records of concrete experiences: individual tool calls, conversation turns, environment observations.
In the Generative Agents world [Park et al., [2023](#bib.bib7)], every observation—“Isabella saw Klaus painting in the park at 3pm”—lands in the episodic stream with a timestamp, an importance score, and an embedding for later retrieval.
...
Voyager’s skill library [Wang et al., [2023a](#bib.bib8)] is the clearest example: every verified Minecraft routine is stored as runnable JavaScript, indexed by a natural language description, and composed on the fly for novel tasks.
...
Context-resident text—summaries, scratchpads, chain-of-thought traces [Wei et al., [2022](#bib.bib5)]—is the simplest substrate.
Fully transparent, zero infrastructure, but ruthlessly capacity-limited.
Vector-indexed stores encode records as dense embeddings and support approximate nearest-neighbor search [Karpukhin et al., [2020](#bib.bib36), Johnson et al., [2021](#bib.bib37)].
...
Structured stores—SQL databases [Hu et al., [2023](#bib.bib16)], key–value maps, knowledge graphs [Ji et al., [2022](#bib.bib38)]—preserve relational structure and support complex queries (“all API failures involving service X in the last 7 days”), at the cost of upfront schema design.
Executable repositories—code libraries, tool definitions, plan templates [Wang et al., [2023a](#bib.bib8)]—let the agent invoke stored skills directly, sidestepping regeneration and the errors it introduces.
Hybrid stores are the norm in production.
MemGPT [Packer et al., [2024](#bib.bib10)], for instance, layers a context-window “main memory” over a searchable recall database and a vector-indexed archive—each tier with different access patterns and eviction rules.
3.3 Control policy
...
Heuristic control hard-codes rules: top- retrieval, summarize every turns, expire records older than days. Predictable, easy to debug, but blind to context.
Prompted self-control exposes memory operations as tool calls and lets the LLM decide when to invoke them.
MemGPT’s core_memory_append and archival_memory_search are canonical examples [Packer et al., [2024](#bib.bib10)].
...
Agentic Memory [Yu et al., [2026](#bib.bib11)] trains store, retrieve, update, summarize, and discard as callable tools via a three-stage RL pipeline with step-wise GRPO.
...
(i) sliding windows that retain the most recent turns and drop the rest;
...
The Self-Controlled Memory system [Liang et al., [2023](#bib.bib17)] hands this decision to the agent itself, letting it choose which segments deserve verbatim retention versus aggressive condensation.
...
Extending context windows to 100k+ tokens [Chen et al., [2023](#bib.bib48)] delays the problem but cannot eliminate it, and longer contexts incur quadratic cost increases in attention.
...
RAG [Lewis et al., [2020](#bib.bib1)] demonstrated that pairing a generator with a non-parametric retrieval index produces strong results on knowledge-intensive tasks.
...
Dense passage retrieval [Karpukhin et al., [2020](#bib.bib36)] via learned encoders, typically backed by FAISS-style approximate nearest-neighbor search [Johnson et al., [2021](#bib.bib37)], remains the default implementation, often augmented with sparse BM25 and metadata filters (timestamps, tool types, task tags).
Query formulation.
A subtlety that many systems gloss over: the agent’s immediate input is often a poor retrieval query.
...
Self-RAG [Asai et al., [2024](#bib.bib58)] goes one step further and teaches the model to decide whether retrieval is warranted at all—a simple gate that substantially cuts unnecessary latency.
Scale.
RETRO [Borgeaud et al., [2022](#bib.bib2)] and follow-up work on trillion-token datastores [Raad et al., [2024](#bib.bib60)] suggest that retrieval memory can scale to years of interaction history without architectural changes.
...
Read-write memory.
RET-LLM [Sun et al., [2024](#bib.bib55)] bridges free-form retrieval and structured storage by letting the agent write structured triplets at storage time while querying them via natural language.
This is a pragmatic compromise: schema at write time, flexibility at read time.
4.3 Reflective and self-improving memory
Reflexion [Shinn et al., [2023](#bib.bib4)] introduced a deceptively simple idea: after failing a task, have the agent write a natural language post-mortem, then prepend it to the prompt on the next attempt.
...
Generative Agents [Park et al., [2023](#bib.bib7)] built a richer pipeline.
...
ExpeL [Zhao et al., [2024](#bib.bib52)] pushes the paradigm further by systematically contrasting successful and failed trajectories, extracting discriminative “rules of thumb,” and storing them as reusable heuristics.
Think-in-Memory [Liu et al., [2024a](#bib.bib14)] separates retrieval from reasoning: the agent first recalls, then performs a dedicated thinking step over the recalled content before generating a response.
...
MemGPT [Packer et al., [2024](#bib.bib10)] borrows an idea that operating system designers perfected decades ago: virtual memory.
...
JARVIS-1 [Wang et al., [2024b](#bib.bib53)] extends the hierarchical principle to multimodal settings, with separate stores for visual observations, textual plans, and executable skills.
Cognitive Architectures for Language Agents [Sumers et al., [2024](#bib.bib15)] propose a generalized blueprint where working, episodic, semantic, and procedural stores interact through a central executive (the LLM), directly echoing Baddeley’s model [Baddeley, [2000](#bib.bib32)].
...
Heuristics and prompted self-control are not optimized for the agent’s end task. A -nearest-neighbor retriever does not know whether the retrieved record will actually help; a fixed summarization schedule does not care whether the material being compressed is important.
Agentic Memory (AgeMem) [Yu et al., [2026](#bib.bib11)] addresses this by treating five memory operations—store, retrieve, update, summarize, discard—as callable tools within the agent’s policy, then optimizing the entire pipeline with reinforcement learning.
...
MemLLM [Modarressi et al., [2024](#bib.bib13)] fine-tunes the LLM to interact with an explicit read-write memory module, tightly coupling parametric and non-parametric knowledge.
Joint training of retrieval and generation [Zhong et al., [2022](#bib.bib67)] yields better memory utilization than frozen-retriever baselines.
...
MemBench [Tan et al., [2025](#bib.bib23)] distinguishes factual from reflective memory and tests each in both participation and observation modes.
...
MemoryAgentBench [Hu et al., [2025](#bib.bib24)] grounds evaluation in cognitive science, probing four competencies: accurate retrieval, test-time learning, long-range understanding, and selective forgetting.
...
MemoryArena [He et al., [2026](#bib.bib25)] embeds memory evaluation inside complete agentic tasks—web navigation, preference-constrained planning, progressive information search, and sequential formal reasoning—where later subtasks depend on what the agent learned from earlier ones.
...
Despite context windows stretching to 200k tokens [Chen et al., [2023](#bib.bib48)], long-context models consistently underperform purpose-built memory systems on tasks requiring selective retrieval and active management.
...
Agents routinely surface plausible but stale or off-topic records [Maharana et al., [2024](#bib.bib22)].
...
MemoryBank [Zhong et al., [2024](#bib.bib12)] models memory decay via Ebbinghaus forgetting curves [Ebbinghaus, [1885](#bib.bib35)]: frequently accessed, high-importance memories are reinforced, while neglected ones fade.
MemGPT [Packer et al., [2024](#bib.bib10)] demonstrates multi-session chat with evolving user models.
...
Coding agents assist with generation, debugging, review, and project management across codebases that may contain millions of lines [Qian et al., [2024](#bib.bib42), Hong et al., [2024](#bib.bib41)].
...
ChatDev [Qian et al., [2024](#bib.bib42)] equips role-playing agents (CEO, CTO, programmer, tester) with shared memory to keep a project coherent across development phases.
MetaGPT [Hong et al., [2024](#bib.bib41)] structures this shared memory as standardized documents—PRDs, design specs, code modules—that persist and evolve.
...
Voyager [Wang et al., [2023a](#bib.bib8)] showed that an ever-growing skill library enables lifelong learning: 3.3 more unique items and 15.3 faster milestone progression than prior agents.
JARVIS-1 [Wang et al., [2024b](#bib.bib53)] extends this with multimodal memory spanning visual observations and textual plans.
Ghost in the Minecraft [Zhu et al., [2023](#bib.bib45)] uses text-based knowledge and memory for generally capable open-world agents.
...
AutoGen [Wu et al., [2023](#bib.bib46)] lets agents build on each other’s contributions through shared context.
CAMEL [Li et al., [2024](#bib.bib47)] explores role-aware communicative agents that must remember prior agreements and collaborative history.
ProAgent [Zhang et al., [2024a](#bib.bib54)] builds proactive teammates that anticipate needs based on memory of past interactions.
...
Tool-using agents [Schick et al., [2024](#bib.bib44)] interact with APIs, databases, and web services.
...
AgentBench [Liu et al., [2023](#bib.bib43)] evaluates agents across eight environments; agents that lose track of their command history show sharp performance drops in multi-step tasks.
DERA [Nair et al., [2023](#bib.bib59)] uses dialog-turn memory to refine tool-use strategies iteratively.
...
Tree of Thoughts [Yao et al., [2024](#bib.bib6)] provides a framework for deliberate problem-solving that could benefit from cross-domain procedural memory.
...
two-stage retrieval (fast BM25 or metadata filter slower cross-encoder reranker),
retrieval-or-not gating [Asai et al., [2024](#bib.bib58)],
...
Xu et al. [Xu et al., [2024](#bib.bib49)] show that retrieving a handful of highly relevant passages into a moderate-length context often beats both pure long-context and pure retrieval approaches—a useful guideline for tuning the latency–quality tradeoff.
...
Machine unlearning [Bourtoule et al., [2021](#bib.bib50), Liu et al., [2024b](#bib.bib51)] is the only path, and it remains far from production-ready.
...
MemGPT [Packer et al., [2024](#bib.bib10)] and AgeMem [Yu et al., [2026](#bib.bib11)] are exemplars.
...
Xi et al. [Xi et al., [2023](#bib.bib19)] and Wang et al. [Wang et al., [2024a](#bib.bib20)] offer broad agent surveys in which memory is one module among many.
Zhang et al. [Zhang et al., [2024b](#bib.bib18)] focus specifically on memory and organize their review around write–manage–read operations; our work updates their coverage with 2025–2026 systems (AgeMem, MemBench, MemoryAgentBench, MemoryArena), adds a POMDP-grounded formulation, and extends the discussion to applications, engineering patterns, and governance.
Gao et al. [Gao et al., [2024](#bib.bib21)] survey RAG comprehensively, but their scope is the retrieval–generation pipeline, not agent-specific memory needs.
Sumers et al. [Sumers et al., [2024](#bib.bib15)] propose a cognitive architecture blueprint for language agents; our taxonomy is complementary, sharing the cognitive science terminology but extending the analysis to representational substrates and control policies.
A question often raised is whether expanding context windows—from 4k in early GPT models to 200k+ today [Chen et al., [2023](#bib.bib48)]—makes external memory obsolete.
...
Moreover, inference cost scales quadratically with context length, and Xu et al. [Xu et al., [2024](#bib.bib49)] show empirically that a modest context augmented with targeted retrieval outperforms brute-force long context on many tasks.
9 Open Challenges
...
Neuroscience offers a suggestive model: during sleep, the hippocampus replays recent experiences, strengthening important traces and pruning the rest [Squire, [2004](#bib.bib33)].
...
This mirrors the hippocampal-to-neocortical transfer observed in biological memory [Squire, [2004](#bib.bib33)], where new memories are initially hippocampus-dependent and gradually become independent through repeated reactivation.
...
Self-reflection is a powerful adaptation mechanism that can also entrench mistakes. If the agent falsely concludes that “approach always fails,” it will never test approach again—a classic confirmation bias. Future systems need external validation (check reflections against ground truth when available), uncertainty quantification (decay confidence over time without confirming evidence), adversarial probing (periodically challenge stored beliefs with counterexamples), and expiration policies (retire unvalidated reflections after a set period).
...
Connections to machine unlearning [Bourtoule et al., [2021](#bib.bib50), Liu et al., [2024b](#bib.bib51)] are critical when memories have influenced model behavior through in-context learning or fine-tuning.
...
JARVIS-1 [Wang et al., [2024b](#bib.bib53)] provides an early example in Minecraft, but real-world embodied settings add spatial memory, real-time latency constraints, and the thorny problem of cross-modal retrieval—finding a visual memory via a textual query, or vice versa.
...
Sparse retrieval (activating a tiny fraction of the store per step), compressed session vectors, memory-native architectures like Recurrent Memory Transformers [Bulatov et al., [2022](#bib.bib65)], and retrieval-free injection via adapters [Modarressi et al., [2024](#bib.bib13)] all point toward cheaper alternatives, though none has yet demonstrated strong agent-level performance.
...
Spreading activation [Anderson, [1983](#bib.bib34)]—where accessing one memory primes related ones—could improve retrieval beyond direct similarity.
Memory reconsolidation theory—retrieval renders a memory labile and subject to revision—could inform update mechanisms.
Ebbinghaus curves [Ebbinghaus, [1885](#bib.bib35)], already used in MemoryBank [Zhong et al., [2024](#bib.bib12)], could be extended with spaced repetition to optimize reinforcement timing.
9.9 Foundation models for memory management
A longer-term vision: a foundation model for memory control, trained across diverse agent tasks to perform write, retrieve, summarize, forget, and consolidate operations with general competence—much as instruction-tuned LLMs [Ouyang et al., [2022](#bib.bib40)] provide general language capabilities.
AgeMem [Yu et al., [2026](#bib.bib11)] takes a first step by learning memory management as a policy, but the vision of a truly task-agnostic memory controller remains unrealized.
...
Cited by:
[§4.2](#S4.SS2.p3.1),[§7.2](#S7.SS2.p1.1). -
...
Cited by:
[§3.1](#S3.SS1.p1.1),[§3](#S3.p1.1),[§4.4](#S4.SS4.p2.1). -
Improving language models by retrieving from trillions of tokens.
In Proceedings of the 39th International Conference on Machine Learning (ICML),
Cited by:
[§1.2](#S1.SS2.p2.1),[Table 1](#S3.T1.2.5.2.1.1.1),[§4.2](#S4.SS2.p4.1). -
Machine unlearning.
IEEE Symposium on Security and Privacy.
Cited by:
[§7.5](#S7.SS5.p2.1),[§9.4](#S9.SS4.p1.1). -
...
Cited by:
[§1.3](#S1.SS3.p1.1),[§2.4](#S2.SS4.p1.1),[Table 1](#S3.T1.2.18.15.1.1.1),[§5.2](#S5.SS2.p5.1). -
...
Cited by:
[§1.2](#S1.SS2.p3.1),[§3.2](#S3.SS2.p4.1),[Table 1](#S3.T1.2.10.7.1.1.1). -
Evaluating memory in LLM agents via incremental multi-turn interactions.
arXiv preprint arXiv:2507.05257.
Cited by:
[§1.3](#S1.SS3.p1.1),[Table 1](#S3.T1.2.16.13.1.1.1),[§5.2](#S5.SS2.p4.1). -
...
Cited by:
[§3.2](#S3.SS2.p3.1),[§4.2](#S4.SS2.p2.1). -
...
Cited by:
[§1.2](#S1.SS2.p2.1),[Table 1](#S3.T1.2.4.1.1.1.1),[§4.2](#S4.SS2.p1.1). -
...
Cited by:
[Table 1](#S3.T1.2.14.11.1.1.1),[§5.2](#S5.SS2.p2.1),[§5.5](#S5.SS5.p3.1). -
MemLLM: finetuning LLMs to use an explicit read-write memory.
arXiv preprint arXiv:2404.11672.
Cited by:
[§4.6](#S4.SS6.p1.1),[§9.7](#S9.SS7.p1.1). -
...
Cited by:
[§1.2](#S1.SS2.p3.1),[§3.2](#S3.SS2.p6.1),[§3.3](#S3.SS3.p3.1),[Table 1](#S3.T1.2.12.9.1.1.1),[§4.4](#S4.SS4.p1.1),[§6.1](#S6.SS1.p1.1),[§7.6](#S7.SS6.p4.1). -
Generative agents: interactive simulacra of human behavior.
In Proceedings of the 36th Annual ACM Symposium on User Interface Software and Technology (UIST),
Cited by:
[§1.2](#S1.SS2.p3.1),[§2.4](#S2.SS4.p1.1),[§3.1](#S3.SS1.p2.1),[Table 1](#S3.T1.2.8.5.1.1.1),[§4.3](#S4.SS3.p2.1). -
...
Cited by:
[§1.3](#S1.SS3.p1.1),[Table 1](#S3.T1.2.15.12.1.1.1),[§5.2](#S5.SS2.p3.1). -
...
Cited by:
[§1.2](#S1.SS2.p3.1),[§2.4](#S2.SS4.p1.1),[§3.1](#S3.SS1.p4.1),[§3.2](#S3.SS2.p5.1),[Table 1](#S3.T1.2.2.3.1.1),[§6.3](#S6.SS3.p1.2). -
A survey on large language model based autonomous agents.
Frontiers of Computer Science 18 (6), pp. 186345.
Cited by:
[§1.3](#S1.SS3.p1.1),[§8](#S8.p1.1). -
Augmenting language models with long-term memory.
In Advances in Neural Information Processing Systems (NeurIPS),
Cited by:
[Table 1](#S3.T1.2.9.6.1.1.1). -
JARVIS-1: open-world multi-task agents with memory-augmented multimodal language models.
arXiv preprint arXiv:2311.05997.
Cited by:
[§4.4](#S4.SS4.p2.1),[§6.3](#S6.SS3.p1.2),[§9.5](#S9.SS5.p1.1). -
...
Cited by:
[§1.2](#S1.SS2.p3.1),[Table 1](#S3.T1.2.6.3.1.1.1). -
Agentic memory: learning unified long-term and short-term memory management for large language model agents.
arXiv preprint arXiv:2601.01885.
Cited by:
[§1.2](#S1.SS2.p3.1),[§1.3](#S1.SS3.p1.1),[§3.3](#S3.SS3.p4.1),[Table 1](#S3.T1.2.17.14.1.1.1),[§4.5](#S4.SS5.p2.1),[§7.6](#S7.SS6.p4.1),[§9.9](#S9.SS9.p1.1). -
...
Cited by:
[§1.1](#S1.SS1.p2.1),[§1.3](#S1.SS3.p1.1),[§8](#S8.p1.1). -
ExpeL: LLM agents are experiential learners.
Proceedings of the AAAI Conference on Artificial Intelligence 38.
Cited by:
[Table 1](#S3.T1.2.11.8.1.1.1),[§4.3](#S4.SS3.p3.1). -
...
Proceedings of the AAAI Conference on Artificial Intelligence 38.
Cited by:
[Table 1](#S3.T1.2.13.10.1.1.1),[§6.1](#S6.SS1.p1.1),[§9.8](#S9.SS8.p1.1). -
