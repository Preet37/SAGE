# Source: https://d2l.ai/chapter_attention-mechanisms-and-transformers/transformer.html
# Author: Dive into Deep Learning
# Author Slug: dive-into-deep-learning
# Title: Dive into Deep Learning — The Transformer
# Fetched via: trafilatura
# Date: 2026-04-11

11.7. The Transformer Architecture[¶](#the-transformer-architecture)[ ](https://studiolab.sagemaker.aws/import/github/d2l-ai/d2l-pytorch-sagemaker-studio-lab/blob/main/GettingStarted-D2L.ipynb) Open the notebook in SageMaker Studio Lab
We have compared CNNs, RNNs, and self-attention in
[Section 11.6.2](self-attention-and-positional-encoding.html#subsec-cnn-rnn-self-attention). Notably, self-attention
...
([Cheng et al., 2016](../chapter_references/zreferences.html#id38), [Lin et al., 2017](../chapter_references/zreferences.html#id169), [Paulus et al., 2017](../chapter_references/zreferences.html#id210)),
the Transformer model is solely based on attention mechanisms without
any convolutional or recurrent layer
([Vaswani et al., 2017](../chapter_references/zreferences.html#id302)). Though originally proposed for
...
[Fig. 11.7.1](#fig-transformer). As we can see, the Transformer is composed
of an encoder and a decoder. In contrast to Bahdanau attention for
sequence-to-sequence learning in [Fig. 11.4.2](bahdanau-attention.html#fig-s2s-attention-details),
...
[Fig. 11.7.1](#fig-transformer). At a high level, the Transformer encoder is
...
encoder layer. Inspired by the ResNet design of [Section 8.6](../chapter_convolutional-modern/resnet.html#sec-resnet),
...
followed by layer normalization ([Ba et al., 2016](../chapter_references/zreferences.html#id9)). As a
...
scaled dot products in [Section 11.5](multihead-attention.html#sec-multihead-attention) and
positional encoding in [Section 11.6.3](self-attention-and-positional-encoding.html#subsec-positional-encoding). In the
following, we will implement the rest of the Transformer model.
11.7.2. Positionwise Feed-Forward Networks[¶](#positionwise-feed-forward-networks)
...
positionwise. In the implementation below, the input X
with shape
...
11.7.3. Residual Connection and Layer Normalization[¶](#residual-connection-and-layer-normalization)
Now let’s focus on the “add & norm” component in
[Fig. 11.7.1](#fig-transformer). As we described at the beginning of this
...
In [Section 8.5](../chapter_convolutional-modern/batch-norm.html#sec-batch-norm), we explained how batch normalization
recenters and rescales across the examples within a minibatch. As
discussed in [Section 8.5.2.3](../chapter_convolutional-modern/batch-norm.html#subsec-layer-normalization-in-bn), layer
...
class using a residual connection
...
following TransformerEncoderBlock
class contains two sublayers:
...
(2, 100, 24))
11.7.5. Decoder[¶](#decoder)
As shown in [Fig. 11.7.1](#fig-transformer), the Transformer decoder is
composed of multiple identical layers. Each layer is implemented in the
following TransformerDecoderBlock
class, which contains three
...
so that any query only attends to all
...
feature dimension (num_hiddens
) of the decoder is the same as that
of the encoder.
...
. In the end, a fully connected
layer computes the prediction for all the vocab_size
possible output
...
[Section 10.7.6](../chapter_recurrent-modern/seq2seq.html#sec-seq2seq-training), we train the Transformer model for
...
How would you improve the computational and memory efficiency of Transformers? Hint: you may refer to the survey paper by Tay et al. (
[2020](../chapter_references/zreferences.html#id280)).