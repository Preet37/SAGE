...
1 Introduction
For the Transformer architecture [[56](https://arxiv.org/html/2407.08608v1#bib.bib56)], the attention mechanism constitutes the primary computational bottleneck, since computing the self-attention scores of queries and keys has quadratic scaling in the sequence length.
Scaling attention to longer context will unlock new capabilities (modeling and
reasoning over multiple long
documents [[24](https://arxiv.org/html/2407.08608v1#bib.bib24), [49](https://arxiv.org/html/2407.08608v1#bib.bib49), [42](https://arxiv.org/html/2407.08608v1#bib.bib42)] and files in
large codebases [[47](https://arxiv.org/html/2407.08608v1#bib.bib47), [30](https://arxiv.org/html/2407.08608v1#bib.bib30)]), new modalities (high-resolution
images [[11](https://arxiv.org/html/2407.08608v1#bib.bib11)], audio [[23](https://arxiv.org/html/2407.08608v1#bib.bib23)], video [[25](https://arxiv.org/html/2407.08608v1#bib.bib25)]), and new applications (user interaction with long history [[51](https://arxiv.org/html/2407.08608v1#bib.bib51)],
agent workflow with long horizon [[59](https://arxiv.org/html/2407.08608v1#bib.bib59)]).
This has generated significant interest in making attention faster in the long-context regime, including
by approximation [[27](https://arxiv.org/html/2407.08608v1#bib.bib27), [14](https://arxiv.org/html/2407.08608v1#bib.bib14), [54](https://arxiv.org/html/2407.08608v1#bib.bib54)] and software optimization
([[44](https://arxiv.org/html/2407.08608v1#bib.bib44), [17](https://arxiv.org/html/2407.08608v1#bib.bib17), [29](https://arxiv.org/html/2407.08608v1#bib.bib29)]), or even alternative
architectures [[41](https://arxiv.org/html/2407.08608v1#bib.bib41), [53](https://arxiv.org/html/2407.08608v1#bib.bib53), [22](https://arxiv.org/html/2407.08608v1#bib.bib22)].
In this work, we build on the work of Dao et al. [[17](https://arxiv.org/html/2407.08608v1#bib.bib17)] on developing exact-attention algorithms that integrate knowledge of the GPU’s execution model and hardware characteristics into their high-level design.
In [[17](https://arxiv.org/html/2407.08608v1#bib.bib17)], Dao et al. introduced FlashAttention, a novel tiling strategy for parallelizing attention that eliminates intermediate reads/writes to slow global memory through fusing all of the attention operations into a single GPU kernel.
Dao [[15](https://arxiv.org/html/2407.08608v1#bib.bib15)] restructured the algorithm as FlashAttention-2 to also parallelize over the sequence length dimension and perform the inner loop of the forward pass over blocks of the key and value matrices, thus improving the occupancy and distribution of work on the GPU.
...
We review the capabilities afforded by Hopper in these directions in [§ 2.2](https://arxiv.org/html/2407.08608v1#S2.SS2).
...
in LLMs [[20](https://arxiv.org/html/2407.08608v1#bib.bib20), [52](https://arxiv.org/html/2407.08608v1#bib.bib52)].
...
Memory hierarchy:
The GPU’s memories are organized as a hierarchy of data locales, with capacity inversely related to bandwidth ([Table 1](https://arxiv.org/html/2407.08608v1#S2.T1))444Luo et al. [[34](https://arxiv.org/html/2407.08608v1#bib.bib34)] reports shared memory bandwidth of 128 bytes per clock cycle per SM, and we multiply that by 132 SMs and the boost clock of 1830 MHz..
...
For async memory copy between GMEM and SMEM, Hopper has the Tensor Memory Accelerator (TMA) as a dedicated hardware unit [[38](https://arxiv.org/html/2407.08608v1#bib.bib38), §7.29].
Furthermore, unlike prior architectures such as Ampere, the Tensor Core of Hopper, exposed via the warpgroup-wide WGMMA instruction [[39](https://arxiv.org/html/2407.08608v1#bib.bib39), §9.7.14], is also asynchronous and can source its inputs directly from shared memory.
...
Generically, this improves the compiler’s ability to generate optimal instruction schedules [[4](https://arxiv.org/html/2407.08608v1#bib.bib4)].
In addition, Hopper supports the dynamic reallocation of registers between warpgroups via setmaxnreg
[[39](https://arxiv.org/html/2407.08608v1#bib.bib39), §9.7.17.1], so those warps doing MMAs can obtain a larger share of RMEM than those just issuing TMA (for which only a single thread is needed).
...
In the context of attention, these layout restrictions entail certain modifications to the design of an FP8 algorithm, which we describe in [§ 3.3](https://arxiv.org/html/2407.08608v1#S3.SS3).
2.3 Standard Attention and Flash Attention
Following Dao et al. [[17](https://arxiv.org/html/2407.08608v1#bib.bib17)], we let standard attention denote an implementation of attention on the GPU that materializes the intermediate matrices and to HBM. The main idea of FlashAttention was to leverage a local version of the softmax reduction to avoid these expensive intermediate reads/writes and fuse attention into a single kernel. Local softmax corresponds to lines [18](https://arxiv.org/html/2407.08608v1#alg1.l18)-[19](https://arxiv.org/html/2407.08608v1#alg1.l19) of the consumer mainloop in [Algorithm 1](https://arxiv.org/html/2407.08608v1#alg1) together with the rescalings of blocks of . The simple derivation that this procedure indeed computes can be found in [[15](https://arxiv.org/html/2407.08608v1#bib.bib15), §2.3.1].
3 FlashAttention-3: Algorithm
In this section, we describe the FlashAttention-3 algorithm. For simplicity, we focus on the
forward pass, with the backward pass algorithm described in [§ B.1](https://arxiv.org/html/2407.08608v1#A2.SS1). We first indicate how to integrate warp-specialization with a circular SMEM buffer into the base algorithm of FlashAttention-2. We then explain how to exploit asynchrony of WGMMA to define an overlapped GEMM-softmax 2-stage pipeline. Finally, we describe the modifications needed for FP8, both in terms of layout conformance and accuracy via block quantization and incoherent processing.
...
For our implementation of [Algorithm 1](https://arxiv.org/html/2407.08608v1#alg1) on Hopper, we use setmaxnreg
...
For interpreting the execution flow of [Algorithm 1](https://arxiv.org/html/2407.08608v1#alg1), note that issuing TMA loads does not stall on the completion of other loads due to asynchrony.
...
This is illustrated in [Fig. 1](https://arxiv.org/html/2407.08608v1#S3.F1).
...
For example, (local) softmax (lines [18](https://arxiv.org/html/2407.08608v1#alg1.l18) to [19](https://arxiv.org/html/2407.08608v1#alg1.l19)) relies on the output of the first GEMM, while the second GEMM takes its result as an operand.
Indeed, the wait statements in lines [17](https://arxiv.org/html/2407.08608v1#alg1.l17) and [21](https://arxiv.org/html/2407.08608v1#alg1.l21) of [Algorithm 1](https://arxiv.org/html/2407.08608v1#alg1) serialize the execution of softmax and GEMMs.
...
[Algorithm 2](https://arxiv.org/html/2407.08608v1#alg2) functions as a replacement for the consumer path of [Algorithm 1](https://arxiv.org/html/2407.08608v1#alg1) to comprise the complete FlashAttention-3 algorithm for FP16 precision. At a high-level, we use WGMMA as a metonym for asynchronous GEMM. Within the mainloop (lines [8](https://arxiv.org/html/2407.08608v1#alg2.l8) to [16](https://arxiv.org/html/2407.08608v1#alg2.l16)), the second WGMMA operation of iteration (line [11](https://arxiv.org/html/2407.08608v1#alg2.l11)) is overlapped with softmax operations from iteration (line [13](https://arxiv.org/html/2407.08608v1#alg2.l13)).
...
A detailed description of the 3-stage algorithm and its evaluation results can be found in [§ B.3](https://arxiv.org/html/2407.08608v1#A2.SS3).
...
kernel777An optimized transpose kernel will achieve speed near the bandwidth of the device [[45](https://arxiv.org/html/2407.08608v1#bib.bib45)].
...
and storing RMEM to SMEM at a granularity of 128 bytes.888In the PTX documentation, LDSM/STSM are described as copying matrices with 16-bit entries [[39](https://arxiv.org/html/2407.08608v1#bib.bib39), §9.7.13.4.15-16],
...
We depict fragments of these two layouts in [Fig. 3](https://arxiv.org/html/2407.08608v1#S3.F3) and [Fig. 4](https://arxiv.org/html/2407.08608v1#S3.F4),
...
Moreover, large models typically have outlier values [[20](https://arxiv.org/html/2407.08608v1#bib.bib20), [52](https://arxiv.org/html/2407.08608v1#bib.bib52)] that are much larger in magnitude than most other values,
making quantization difficult.
One typically use per-tensor scaling [[37](https://arxiv.org/html/2407.08608v1#bib.bib37)] by keeping one scalar per tensor (e.g., one
for , for , and for ).
...
[9](https://arxiv.org/html/2407.08608v1#bib.bib9)] and choose to be the product of random diagonal matrices of and a Hadamard matrix, which can be multiplied in instead of , and can also be fused with the rotary embedding at no extra computation cost.
We validate that these two techniques reduces numerical error by up to 2.6 in [§ 4.3](https://arxiv.org/html/2407.08608v1#S4.SS3).
4 Empirical Validation
We use the primitives from CUTLASS [[55](https://arxiv.org/html/2407.08608v1#bib.bib55)] such as WGMMA
...
in [Fig. 5](https://arxiv.org/html/2407.08608v1#S4.F5) and [Fig. 6](https://arxiv.org/html/2407.08608v1#S4.F6), showing that FlashAttention-3 is around 1.5-2.0 faster
...
We report the results for headdim 256 in [Fig. 7](https://arxiv.org/html/2407.08608v1#S4.F7) and give the full results in [§ C.2](https://arxiv.org/html/2407.08608v1#A3.SS2).
...
The result in [Table 2](https://arxiv.org/html/2407.08608v1#S4.T2) confirms that our algorithmic improvements (asynchrony with warp-specialization and overlapping between GEMM and softmax) lead to significant speedup, from 570 to 661 TFLOPs.
...
As there has been interest in the numerical error [[21](https://arxiv.org/html/2407.08608v1#bib.bib21)] of
...
To simulate outlier features and activations in LLMs [[20](https://arxiv.org/html/2407.08608v1#bib.bib20), [52](https://arxiv.org/html/2407.08608v1#bib.bib52)], we generate the entries of with the following
distribution:
...
We then measure the root mean squared error (RMSE) in [Table 3](https://arxiv.org/html/2407.08608v1#S4.T3).
...
Acknowledgments
We are grateful to the NVIDIA CUTLASS team (especially Haicheng Wu, Aniket Shivam, and Cris Cecka) for helping us understand Hopper’s programming model and for their library, which provides clean and powerful building blocks for the implementation of FlashAttention-3. We thank the cuDNN team for the idea of in-kernel transpose for FP8. We appreciate Driss Guessous for integrating FlashAttention to PyTorch. FlashAttention-3 has benefited from insightful discussions with Horace He on different attention variants, with Hao Liu and Phil Wang on distributed attention, and with Daniel Haziza and Chris De Sa on quantization. We thank Meta, Together AI, and Princeton Language and Intelligence (PLI) for compute support.
References
...
URL
[https://github.com/NVIDIA/cutlass](https://github.com/NVIDIA/cutlass). - Vaswani et al. [2017] Ashish Vaswani, Noam Shazeer, Niki Parmar, Jakob Uszkoreit, Llion Jones, Aidan N Gomez, Łukasz Kaiser, and Illia Polosukhin. Attention is all you need. Advances in neural information processing systems, 30, 2017.
...
architecture [[56](https://arxiv.org/html/2407.08608v1#bib.bib56)], there has been a large body of work
...
either with a fixed pattern [[12](https://arxiv.org/html/2407.08608v1#bib.bib12)], with a sliding
window [[6](https://arxiv.org/html/2407.08608v1#bib.bib6)], or with a dynamic pattern through
hashing [[28](https://arxiv.org/html/2407.08608v1#bib.bib28)] or routing [[46](https://arxiv.org/html/2407.08608v1#bib.bib46)].
...
key [[27](https://arxiv.org/html/2407.08608v1#bib.bib27)] with random
projection [[13](https://arxiv.org/html/2407.08608v1#bib.bib13), [43](https://arxiv.org/html/2407.08608v1#bib.bib43), [58](https://arxiv.org/html/2407.08608v1#bib.bib58)].
One can also combine the sparse and low-rank approximation for better
quality [[60](https://arxiv.org/html/2407.08608v1#bib.bib60), [10](https://arxiv.org/html/2407.08608v1#bib.bib10)].
However, these approximation methods typically do not offer the same model
quality as standard attention [[54](https://arxiv.org/html/2407.08608v1#bib.bib54)], and so most large-scale
models do not employ these techniques.
There are other variants of attention aimed at reducing the size of the KV cache
to improve inference efficiency. Multi-query attention [[50](https://arxiv.org/html/2407.08608v1#bib.bib50)] and grouped query
attention [[3](https://arxiv.org/html/2407.08608v1#bib.bib3)] tie different heads of and , and
multiple query heads interact with the same key and value head.
Multi-head latent attention [[19](https://arxiv.org/html/2407.08608v1#bib.bib19)] parameterizes the and
...
Methods such as Ring attention [[31](https://arxiv.org/html/2407.08608v1#bib.bib31), [32](https://arxiv.org/html/2407.08608v1#bib.bib32)] and
variants [[8](https://arxiv.org/html/2407.08608v1#bib.bib8)] can reach a context length of up to 1
million.
...
attention [[27](https://arxiv.org/html/2407.08608v1#bib.bib27)] and recurrent neural networks
(RNNs).
RWKV [[41](https://arxiv.org/html/2407.08608v1#bib.bib41)], H3 [[18](https://arxiv.org/html/2407.08608v1#bib.bib18)], MEGA [[35](https://arxiv.org/html/2407.08608v1#bib.bib35)],
Retnet [[53](https://arxiv.org/html/2407.08608v1#bib.bib53)] enhance the expressivity of the simple
cumulative sum in linear attention with more sophisticated recurrences.
Mamba [[22](https://arxiv.org/html/2407.08608v1#bib.bib22)] and xLSTM [[5](https://arxiv.org/html/2407.08608v1#bib.bib5)] use learnable
...
the lens of the structure of the token-mixing matrix [[16](https://arxiv.org/html/2407.08608v1#bib.bib16)].
These models have started to see some traction, seeing usage in some medium to
large-scale models such as Jamba [[2](https://arxiv.org/html/2407.08608v1#bib.bib2)], Zamba [[61](https://arxiv.org/html/2407.08608v1#bib.bib61)],
Megalodon [[36](https://arxiv.org/html/2407.08608v1#bib.bib36)], and Mamba2-hybrid [[57](https://arxiv.org/html/2407.08608v1#bib.bib57)].
...
QuIP [[9](https://arxiv.org/html/2407.08608v1#bib.bib9)] uses incoherent processing to reduce the quantization,
...
even 2-bits [[26](https://arxiv.org/html/2407.08608v1#bib.bib26), [33](https://arxiv.org/html/2407.08608v1#bib.bib33)].
...
A recent example of this is LeanAttention [[48](https://arxiv.org/html/2407.08608v1#bib.bib48)],
...
load balancing [[40](https://arxiv.org/html/2407.08608v1#bib.bib40)] to achieve nearly peak occupancy.
There is a large literature on optimizing GEMM for specific hardware that employs
many of the same techniques.
As an example, Abdelfattah et al. [[1](https://arxiv.org/html/2407.08608v1#bib.bib1)] presents a high performance batched GEMM kernel on
...
Similar to the forward pass [§ 3.1](https://arxiv.org/html/2407.08608v1#S3.SS1), we use warp specialization to
handle asynchrony.
...
computation (matmul).
We include the backward pass with warp specialization in [Algorithm 3](https://arxiv.org/html/2407.08608v1#alg3).
...
We experiment with a 3-stage pipelining algorithm to parallelize the first WGMMA from iteration , softmax from iteration , and the second WGMMA from iteration . We describe this algorithm in [Algorithm 4](https://arxiv.org/html/2407.08608v1#alg4).
