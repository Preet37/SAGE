...
We introduce WebArena, a realistic and reproducible web environment designed to facilitate the development of autonomous agents capable of executing tasks (§[2](https://arxiv.org/html/2307.13854v4#S2)). An overview of WebArena is in [Figure 1](https://arxiv.org/html/2307.13854v4#S1.F1).
...
Lastly, WebArena is complemented by an extensive collection of documentation and knowledge bases that vary from general resources like English Wikipedia to more domain-specific references, such as manuals for using the integrated development tool (Fan et al., [2022](https://arxiv.org/html/2307.13854v4#bib.bib8)).
...
We deliver the hosting services using Docker containers with gym-APIs (Brockman et al., [2016](https://arxiv.org/html/2307.13854v4#bib.bib5)), ensuring both the usability and the reproducibility of WebArena.
Along with WebArena, we release a ready-to-use benchmark with 812 long-horizon web-based tasks (§[3](https://arxiv.org/html/2307.13854v4#S3)).
Each task is described as a high-level natural language intent, emulating the abstract language usage patterns typically employed by humans (Bisk et al., [2019](https://arxiv.org/html/2307.13854v4#bib.bib3)). Two example intents are shown in the upper left of [Figure 1](https://arxiv.org/html/2307.13854v4#S1.F1).
We focus on evaluating the functional correctness of these tasks, i.e., does the result of the execution actually achieve the desired goal (§[3.2](https://arxiv.org/html/2307.13854v4#S3.SS2)).
For instance, to evaluate the example in [Figure 2](https://arxiv.org/html/2307.13854v4#S2.F2), our evaluation method verifies the concrete contents in the designated repository.
...
We use this benchmark to evaluate several agents that can follow NL command and perform web-based tasks (§[4](https://arxiv.org/html/2307.13854v4#S4)).
...
We hypothesize that the limited performance of current LLMs stems from a lack of crucial capabilities such as active exploration and failure recovery to successfully perform complex tasks (§[5.1](https://arxiv.org/html/2307.13854v4#S5.SS1)).
These outcomes underscore the necessity for further development towards robust and effective agents (LeCun, [2022](https://arxiv.org/html/2307.13854v4#bib.bib18)) in WebArena.
...
The WebArena environment is denoted as with state space , action space (§[2.4](https://arxiv.org/html/2307.13854v4#S2.SS4)) and observation space (§[2.3](https://arxiv.org/html/2307.13854v4#S2.SS3)).
The transition function is deterministic, and it is defined by the underlying implementation of each website in the environment.
Given a task described as a natural language intent , an agent issues an action based on intent , the current observation , the action history and the observation history .
Consequently, the action results in a new state and its corresponding observation .
We propose a reward function to measure the success of a task execution, where represents the sequence of actions from start to the end time step , and denotes all intermediate states.
...
Then we imported sampled data from their real-world counterparts. As an example, our version of GitLab was developed based on the actual GitLab project.111[https://gitlab.com/gitlab-org/gitlab](https://gitlab.com/gitlab-org/gitlab) We carefully emulated the features of a typical code repository by including both popular projects with many issues and pull requests and smaller, personal projects.
Details of all websites in WebArena can be found in Appendix [A.1](https://arxiv.org/html/2307.13854v4#A1.SS1).
We deliver the environment as dockers and provide scripts to reset the environment to a deterministic initial state (See Appendix [A.2](https://arxiv.org/html/2307.13854v4#A1.SS2)).
...
We provide flexible configuration to render the page content in many modes: (see [Figure 3](https://arxiv.org/html/2307.13854v4#S2.F3) for an example):
(1) the raw web page HTML, composed of a Document Object Model (DOM) tree, as commonly used in past work (Shi et al., [2017](https://arxiv.org/html/2307.13854v4#bib.bib35); Deng et al., [2023](https://arxiv.org/html/2307.13854v4#bib.bib7); Li et al., [2020](https://arxiv.org/html/2307.13854v4#bib.bib21)); (2) a screenshot, a pixel-based representation that represents the current web page as an RGB array and (3) the accessibility tree of the web page.222[https://developer.mozilla.org/en-US/docs/Glossary/Accessibility_tree](https://developer.mozilla.org/en-US/docs/Glossary/Accessibility_tree)
The accessibility tree is a subset of the DOM tree with elements that are relevant and useful for displaying the contents of a web page.
Every element is represented as its role (e.g., a link), its text content, and its properties (e.g., whether it is focusable).
Accessibility trees largely retain the structured information of a web page while being more compact than the DOM representation.
...
We also provided a prompt for the annotators to use with ChatGPT333[https://chat.openai.com/](https://chat.openai.com/) for inspiration,
...
| tab_focus(index) | focus on -th tab |
...
The intent distribution is shown in [Figure 6](https://arxiv.org/html/2307.13854v4#A1.F6).
Furthermore, we classify the intents into three primary categories with examples shown in [Figure 5](https://arxiv.org/html/2307.13854v4#S3.F5):
-
(1)
Information-seeking tasks expect a textual response. Importantly, these tasks in WebArena often require navigation across multiple pages or focus on user-centric content. This makes them distinct from open-domain question-answering (Yang et al.,
...
To measure the correctness of information-seeking tasks where a textual answer is expected, we provide the annotated answer for each intent. The is further compared with the predicted answer with one of the following scoring functions .
First, we define exact_match where only that is identical with receives a score of one. This function is primarily applicable to intent types whose responses follow a more standardized format, similar to the evaluation on question answering literature (Rajpurkar et al., [2016](https://arxiv.org/html/2307.13854v4#bib.bib32); Yang et al., [2018](https://arxiv.org/html/2307.13854v4#bib.bib45)).
Second, we create must_include where any containing receives a score of one. This function is primarily used in when an unordered list of text is expected or where the emphasis of evaluation is on certain key concepts. In the second example in [Table 1](https://arxiv.org/html/2307.13854v4#S3.T1), we expect both the correct name and the email address to be presented, irrespective of the precise wording used to convey the answer.
Finally, we introduce fuzzy_match where we utilize a language model to assess whether is semantically equivalent to .
Specifically, in this work, we use gpt-4-0613 to perform this evaluation.
The corresponding prompt details are provided in Appendix [A.7](https://arxiv.org/html/2307.13854v4#A1.SS7).
The fuzzy_match function applies to situations where the format of the answer is diverse. For instance, in responding to “Compare the time for walking and driving route from AMC Waterfront to Randyland”, it is essential to ensure that driving time and walking time are accurately linked with the correct terms.
The fuzzy_match function could also flexibly match the time “2h58min” with different forms such as “2 hour 58 minutes”, “2:58” and others.
We demonstrate a language model can achieve nearly perfect performance on this task in §[A.8](https://arxiv.org/html/2307.13854v4#A1.SS8).
Evaluating Site Navigation and Content & Config Tasks
The tasks in these categories require accessing web pages that meet certain conditions or performing operations that modify the underlying data storage of the respective websites. To assess these, we establish reward functions that programmatically examine the intermediate states within an execution trajectory to ascertain whether the outcome aligns with the intended result. These intermediate states are often the underlying databases of the websites, the status, and the content of a web page at each step of the execution.
...
For example, the evaluation process for the intent of the fifth example in [Table 1](https://arxiv.org/html/2307.13854v4#S3.T1), first obtains the URL of the latest post by examining the last state in the state sequence . Then it navigates to the corresponding post page and obtains the post’s content by running the Javascript “document.querySelector(‘.submission__inner’).outerText”.
...
| 1 | Tell me the name of the customer who has the most cancellations in the history | exact_match(, “Samantha Jones”) | |
| 2 | Find the customer name and email with phone number 8015551212 | must_include(, “Sean Miller”) | |
| must_include(, “sean@gmail.com”) | |||
| 3 | Compare walking and driving time from AMC Waterfront to Randyland | fuzzy_match(, “walking: 2h58min”) | |
| fuzzy_match(, “driving: 21min”) | |||
| 4 | Checkout merge requests assigned to me | url=locate_current_url() | |
| exact_match(URL, “gitlab.com/merge_ | |||
| requests?assignee_username=byteblaze”) | |||
| 5 | Post to ask “whether I need a car in NYC” | url=locate_latest_post_url( | |
| body=locate_latest_post_body( | |||
| must_include(URL, “/f/nyc”) | |||
| must_include(body,“a car in NYC” ) |
Unachievable Tasks
Due to constraints such as inadequate evidence, user permissions (§[A.3](https://arxiv.org/html/2307.13854v4#A1.SS3)), or the absence of necessary functional support on the website, humans may ask for tasks that are not possible to complete.
Inspired by previous work on evaluating question-answering models on unanswerable questions (Rajpurkar et al., [2018](https://arxiv.org/html/2307.13854v4#bib.bib33)), we design unachievable tasks in WebArena.
...
Annotation Process
The intents were contributed by the authors following the annotation guideline in §[3.1](https://arxiv.org/html/2307.13854v4#S3.SS1). Every author has extensive experience with web-based tasks.
...
In the second setting, with the same information, the model first performs chain-of-thought reasoning steps in the text before the action prediction (CoT, Wei et al. ([2022](https://arxiv.org/html/2307.13854v4#bib.bib43)); Yao et al. ([2022b](https://arxiv.org/html/2307.13854v4#bib.bib47))).
...
The full prompts can be found in Appendix [A.9](https://arxiv.org/html/2307.13854v4#A1.SS9).
The detailed configurations of each model can be found in Appendix [A.6](https://arxiv.org/html/2307.13854v4#A1.SS6).
5 Results
...
The main results are shown on the top of [Table 2](https://arxiv.org/html/2307.13854v4#S5.T2).
GPT-4 (OpenAI, [2023](https://arxiv.org/html/2307.13854v4#bib.bib29)) with CoT prompting achieves a modest end-to-end task success rate of 11.70%, which is significantly lower than the human performance of 78.24%.
GPT-3.5 (OpenAI, [2022](https://arxiv.org/html/2307.13854v4#bib.bib28)) with CoT prompting is only able to successfully perform 8.75% of the tasks.
The explicit reasoning procedure is somewhat helpful, it brings 2.34% improvement over the version without it.
Further, text-bison-001 (Anil et al., [2023](https://arxiv.org/html/2307.13854v4#bib.bib2))
...
As shown in [Table 2](https://arxiv.org/html/2307.13854v4#S5.T2), eliminating this instruction led to a performance boost in achievable tasks, enhancing the overall task success rate of GPT-4 to 14.41%.
Despite an overall decline in identifying unachievable tasks, GPT-4 retains the capacity to recognize 44.44% of such tasks.
It does so by generating reasons of non-achievability, even without explicit instructions.
...
We plot a histogram of per-template success rates for our models in [Table 3](https://arxiv.org/html/2307.13854v4#S5.T3).
...
| Mind2Web | (Deng et al.,
|
[2017](https://arxiv.org/html/2307.13854v4#bib.bib35))
[2018](https://arxiv.org/html/2307.13854v4#bib.bib23))
[2022a](https://arxiv.org/html/2307.13854v4#bib.bib46))
[2020](https://arxiv.org/html/2307.13854v4#bib.bib37))
[2018](https://arxiv.org/html/2307.13854v4#bib.bib31))
[2021](https://arxiv.org/html/2307.13854v4#bib.bib40))
Benchmarks for Controlling Agents through Natural Language
Controlling agents via natural language in the digital world have been studied in the literature (Branavan et al., [2009](https://arxiv.org/html/2307.13854v4#bib.bib4); Shi et al., [2017](https://arxiv.org/html/2307.13854v4#bib.bib35); Liu et al., [2018](https://arxiv.org/html/2307.13854v4#bib.bib23); Toyama et al., [2021](https://arxiv.org/html/2307.13854v4#bib.bib40); Deng et al., [2023](https://arxiv.org/html/2307.13854v4#bib.bib7); Li et al., [2020](https://arxiv.org/html/2307.13854v4#bib.bib21); Xu et al., [2021](https://arxiv.org/html/2307.13854v4#bib.bib44)).
However, the balance between functionality, authenticity, and support for environmental dynamics remains a challenge.
Existing benchmarks often compromise these aspects, as shown in [Table 4](https://arxiv.org/html/2307.13854v4#S6.T4).
Some works rely on static states, limiting agents’ explorations and functional correctness evaluation (Shi et al., [2017](https://arxiv.org/html/2307.13854v4#bib.bib35); Deng et al., [2023](https://arxiv.org/html/2307.13854v4#bib.bib7)), while others simplify real-world complexities, restricting task variety (Yao et al., [2022a](https://arxiv.org/html/2307.13854v4#bib.bib46); Liu et al., [2018](https://arxiv.org/html/2307.13854v4#bib.bib23)).
...
Existing works (Huang et al., [2022](https://arxiv.org/html/2307.13854v4#bib.bib12); Madaan et al., [2022](https://arxiv.org/html/2307.13854v4#bib.bib25); Li et al., [2023](https://arxiv.org/html/2307.13854v4#bib.bib20)) observe LLMs could break a task into more manageable sub-tasks (Zhou et al., [2022b](https://arxiv.org/html/2307.13854v4#bib.bib51)).
This process can be further refined by representing task executions as programs, a technique that aids sub-task management and skill reuse (Zhou et al., [2022a](https://arxiv.org/html/2307.13854v4#bib.bib50); Liang et al., [2023](https://arxiv.org/html/2307.13854v4#bib.bib22); Wang et al., [2023](https://arxiv.org/html/2307.13854v4#bib.bib41); Gao et al., [2023](https://arxiv.org/html/2307.13854v4#bib.bib9)). Meanwhile, search and backtracking methods introduce a more structured approach to planning while also allowing for decision reconsideration (Yao et al., [2023](https://arxiv.org/html/2307.13854v4#bib.bib48); Long, [2023](https://arxiv.org/html/2307.13854v4#bib.bib24)).
Existing works also incorporate failure recovery, self-correction (Shinn et al., [2023](https://arxiv.org/html/2307.13854v4#bib.bib36); Kim et al., [2023](https://arxiv.org/html/2307.13854v4#bib.bib14)), observation summarization (Sridhar et al., [2023](https://arxiv.org/html/2307.13854v4#bib.bib39)) to improve execution robustness.
...
Sünderhauf, Ian D. Reid, Stephen Gould, and Anton van den Hengel.
...
Technologies, Volume 1 (Long and Short Papers), pp. 4077–4085,
Minneapolis, Minnesota, 2019. Association for Computational Linguistics.
doi: 10.18653/v1/N19-1412.
URL
[https://aclanthology.org/N19-1412](https://aclanthology.org/N19-1412). -
Branavan et al. (2009)
...
Language Processing of the AFNLP, pp. 82–90, Suntec, Singapore, 2009.
Association for Computational Linguistics.
URL
[https://aclanthology.org/P09-1010](https://aclanthology.org/P09-1010). -
Brockman et al. (2016)
...
URL
[https://arxiv.org/abs/2107.03374](https://arxiv.org/abs/2107.03374). - Deng et al. (2023) Xiang Deng, Yu Gu, Boyuan Zheng, Shijie Chen, Samuel Stevens, Boshi Wang, Huan Sun, and Yu Su. Mind2web: Towards a generalist agent for the web, 2023.
-
Fan et al. (2022)
...
Jernite et al. (2019)
Yacine Jernite, Kavya Srinet, Jonathan Gray, and Arthur Szlam.
CraftAssist Instruction Parsing: Semantic Parsing for a
Minecraft Assistant.
ArXiv preprint, abs/1905.01978, 2019.
URL
[https://arxiv.org/abs/1905.01978](https://arxiv.org/abs/1905.01978). -
Kim et al. (2023)
Geunwoo Kim, Pierre Baldi, and Stephen McAleer.
Language models can solve computer tasks.
ArXiv preprint, abs/2303.17491, 2023.
URL
[https://arxiv.org/abs/2303.17491](https://arxiv.org/abs/2303.17491). - Kolve et al. (2017) Eric Kolve, Roozbeh Mottaghi, Winson Han, Eli VanderBilt, Luca Weihs, Alvaro Herrasti, Daniel Gordon, Yuke Zhu, Abhinav Gupta, and Ali Farhadi. AI2-THOR: An Interactive 3D Environment for Visual AI. arXiv, 2017.
-
Küttler et al. (2020)
...
URL
[https://proceedings.neurips.cc/paper/2020/hash/569ff987c643b4bedf504efda8f786c2-Abstract.html](https://proceedings.neurips.cc/paper/2020/hash/569ff987c643b4bedf504efda8f786c2-Abstract.html). -
...
URL
[https://aclanthology.org/Q19-1026](https://aclanthology.org/Q19-1026). - LeCun (2022) Yann LeCun. A path towards autonomous machine intelligence version 0.9. 2, 2022-06-27. Open Review, 62, 2022.
...
Take a break in the middle: Investigating subgoals towards
hierarchical script generation.
ArXiv preprint, abs/2305.10907, 2023.
URL
[https://arxiv.org/abs/2305.10907](https://arxiv.org/abs/2305.10907). -
Li et al. (2020)
...
ArXiv preprint, abs/2305.08291, 2023.
URL
...
Natural Language Processing, pp. 1384–1403, Abu Dhabi, United Arab
Emirates, 2022. Association for Computational Linguistics.
URL
[https://aclanthology.org/2022.emnlp-main.90](https://aclanthology.org/2022.emnlp-main.90). - Misra et al. (2016) Dipendra K Misra, Jaeyong Sung, Kevin Lee, and Ashutosh Saxena. Tell me dave: Context-sensitive grounding of natural language to manipulation instructions. The International Journal of Robotics Research, 35(1-3):281–300, 2016.
...
In 2018 IEEE Conference on Computer Vision and Pattern
Recognition, CVPR 2018, Salt Lake City, UT, USA, June 18-22, 2018, pp. 8494–8502. IEEE Computer Society, 2018.
doi: 10.1109/CVPR.2018.00886.
URL
[http://openaccess.thecvf.com/content_cvpr_2018/html/Puig_VirtualHome_Simulating_Household_CVPR_2018_paper.html](http://openaccess.thecvf.com/content_cvpr_2018/html/Puig_VirtualHome_Simulating_Household_CVPR_2018_paper.html). -
...
URL
[https://aclanthology.org/P18-2124](https://aclanthology.org/P18-2124). - Shaw et al. (2023) Peter Shaw, Mandar Joshi, James Cohan, Jonathan Berant, Panupong Pasupat, Hexiang Hu, Urvashi Khandelwal, Kenton Lee, and Kristina Toutanova. From pixels to ui actions: Learning to follow instructions via graphical user interfaces. arXiv preprint arXiv:2306.00245, 2023.
-
Shi et al. (2017)
Tianlin Shi, Andrej Karpathy, Linxi Fan, Jonathan Hernandez, and Percy Liang.
World of bits: An open-domain platform for web-based agents.
In Doina Precup and Yee Whye Teh (eds.), Proceedings of the
34th International Conference on Machine Learning, ICML 2017, Sydney, NSW,
Australia, 6-11 August 2017, volume 70 of Proceedings of Machine
Learning Research, pp. 3135–3144. PMLR, 2017.
URL
[http://proceedings.mlr.press/v70/shi17a.html](http://proceedings.mlr.press/v70/shi17a.html). -
Shinn et al. (2023)
...
URL
[https://doi.org/10.1109/CVPR42600.2020.01075](https://doi.org/10.1109/CVPR42600.2020.01075). -
Shridhar et al. (2021)
...
URL
[https://openreview.net/forum?id=0IOX0YcCdTn](https://openreview.net/forum?id=0IOX0YcCdTn). - Sridhar et al. (2023) Abishek Sridhar, Robert Lo, Frank F Xu, Hao Zhu, and Shuyan Zhou. Hierarchical prompting assists large language model on web navigation. arXiv preprint arXiv:2305.14257, 2023.
-
Toyama et al. (2021)
...
ArXiv preprint, abs/2105.13231, 2021.
URL
...
ArXiv preprint, abs/2212.10481, 2022.
URL
[https://arxiv.org/abs/2212.10481](https://arxiv.org/abs/2212.10481). - Wei et al. (2022) Jason Wei, Xuezhi Wang, Dale Schuurmans, Maarten Bosma, Fei Xia, Ed Chi, Quoc V Le, Denny Zhou, et al. Chain-of-thought prompting elicits reasoning in large language models. Advances in Neural Information Processing Systems, 35:24824–24837, 2022.
-
Xu et al. (2021)
...
Chapter of the Association for Computational Linguistics: Human Language
Technologies, pp. 1022–1032, Online, 2021. Association for Computational
Linguistics.
doi: 10.18653/v1/2021.naacl-main.80.
URL
[https://aclanthology.org/2021.naacl-main.80](https://aclanthology.org/2021.naacl-main.80). -
Yang et al. (2018)
...
Natural Language Processing, pp. 2369–2380, Brussels, Belgium, 2018.
Association for Computational Linguistics.
...
ArXiv preprint, abs/2210.03629, 2022b.
URL
...
models.
ArXiv preprint, abs/2305.10601, 2023.
URL
...
In Proceedings of the Workshop on Structured and Unstructured
Knowledge Integration (SUKI), pp. 67–84, Seattle, USA,
2022a. Association for Computational Linguistics.
doi: 10.18653/v1/2022.suki-1.8.
URL
[https://aclanthology.org/2022.suki-1.8](https://aclanthology.org/2022.suki-1.8). -
Zhou et al. (2022b)
Shuyan Zhou, Li Zhang, Yue Yang, Qing Lyu, Pengcheng Yin, Chris Callison-Burch,
and Graham Neubig.
...
for Computational Linguistics (Volume 1: Long Papers), pp. 2998–3012,
Dublin, Ireland, 2022b. Association for Computational
Linguistics.
doi: 10.18653/v1/2022.acl-long.214.
URL
[https://aclanthology.org/2022.acl-long.214](https://aclanthology.org/2022.acl-long.214).
Appendix A Appendix
A.1 Website Implementation
Given the selected websites described in §[2.2](https://arxiv.org/html/2307.13854v4#S2.SS2), we make the best attempt to reproduce the functionality of commonly used sites in a reproducible way.
...
For the E-commerce category, we constructed a shopping website with approximately 90 products, including the prices, options, detailed product descriptions, images, and reviews, spanning over 300 product categories. This website is developed using Adobe Magento, an open-source e-commerce platform444[https://github.com/magento/magento2](https://github.com/magento/magento2). Data resources were obtained from data from actual online sites, such as that included in the Webshop data dumpYao et al. ([2022a](https://arxiv.org/html/2307.13854v4#bib.bib46)).
As for the social forum platform, we deployed an open-source software Postmill555[https://postmill.xyz/](https://postmill.xyz/), the open-sourced counterpart of Reddit666[https://www.reddit.com/](https://www.reddit.com/).
We sampled from the top 50 subreddits777[https://redditlist.com/sfw.html](https://redditlist.com/sfw.html).
...
We employ OpenStreetMap999[https://www.openstreetmap.org/](https://www.openstreetmap.org/) for map service implementation, confining our focus to the northeast US region due to data storage constraints.
...
Specifically, we utilize Kiwix101010[https://www.kiwix.org/en/](https://www.kiwix.org/en/) to host an offline version of English Wikipedia with a knowledge cutoff of May 2023.
...
As a result, we deploy our websites in separate Docker images 111111[https://www.docker.com/](https://www.docker.com/), one per website.
...
We experiment with GPT-3.5-turbo-16k-0613, GPT-4-0613, and text-bison-001 with a temperature of and a top- parameter of . The maximum number of state transitions is set to 30. We halt execution if the same action is repeated more than three times on the same observation or if the agent generates three consecutive invalid actions. These situations typically indicate a high likelihood of execution failure and hence warrant early termination. For text-bison-001, we additionally allow ten retries until it generates a valid action.
...
In addition, among the 82 examples that require using GPT-4 for evaluation, the answer of 49 (60%) examples is a date (e.g., 10/23/2022) or time duration (e.g., 15 minutes).
In these cases, GPT-4 is only used to judge the different format of the answers.
...
We randomly sample negative examples. For instance, Nov 3, 2022, November 3, 2022, 3rd November 2022, 3 Nov 2022, 2022-11-03, and 3rd of November, 2022 are all correct variances of 2022/11/03. The accuracy of GPT-4 is shown in [Table 6](https://arxiv.org/html/2307.13854v4#A1.T6).
...
We hypothesize that these observed failures are related to the current pretraining and supervised fine-tuning on dialogues employed in GPT models Ouyang et al. ([2022](https://arxiv.org/html/2307.13854v4#bib.bib30)). These models are primarily trained to execute instructions given immediate observations (i.e.,, the dialogue history); thereby, they may exhibit a lack of explorations.
