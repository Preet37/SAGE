...
# Author: LMU Munich NLP Seminar
# Author Slug: lmu-nlp-seminar
...
In 2014, Bahdanau, Cho, and Bengio ([2014](#ref-bahdanau2014neural)) proposed attention to fix the information problem that
...
\(x_1,\dots, x_T\) and \(q\) is a non-linear function. Sutskever, Vinyals, and Le ([2014](#ref-sutskever2014sequence)) for example used
...
Attention changes the context vector \(c\) that a decoder uses for translation from a fixed length vector \(c\) of a sequence of hidden states \(h_1, \dots, h_T\) to a sequence of context vectors \(c_i\). The hidden state \(h_i\) has a strong focus on the i-th word in the input sequence and its surroundings. If a bi-directional encoder is used, each \(h_i\) is computed by a concatenation of the forward \(\overrightarrow{h_i}\) and backward \(\overleftarrow{h_i}\) hidden states:
\[ h_i = [\overrightarrow{h_i}; \overleftarrow{h_i}], i = 1,\dots,n \] These new variable context vectors \(c_i\) are used for the computation of the decoder hidden state \(s_t\). At time-point \(t\) it is computed as \(s_t = f(s_{t-1},y_{t-1},c_t)\) where \(f\) is the function resulting from the use of a LSTM- or GRU-cell. The context vector \(c_t\) is computed as a weighted sum of the hidden states \(h_1,\dots, h_{T_x}\):
\[ c_t = \sum^{T_x}_{i=1}\alpha_{t,i}h_i. \] Each hidden state \(h_i\) is weighted by a \(\alpha_{t,i}\). The weight \(\alpha_{t,i}\) for each hidden state \(h_i\) is also called the alignment score. These alignment scores are computed as:
\[
...
and assigns the weight to \(h_i\). Bahdanau, Cho, and Bengio ([2014](#ref-bahdanau2014neural)) parametrize their alignment
...
is called “concat” in Luong, Pham, and Manning ([2015](#ref-luong2015effective)) and “additive attention” in Vaswani et al. ([2017](#ref-vaswani2017attention))
...
which can be visualised to show the correlation between source and target words as seen in [8.1](attention-and-self-attention-for-nlp.html#fig:attention-plot-bahdanau).
8.1.2 Luong-Attention
While Bahdanau, Cho, and Bengio ([2014](#ref-bahdanau2014neural)) were the first to use attention in neural machine translation,
Luong, Pham, and Manning ([2015](#ref-luong2015effective)) were the first to explore different attention mechanisms and their impact on
...
score(s_t,h_i) = v_\alpha^Ttanh(\mathbf{W}_\alpha[s_t;h_i]),
\]
while Luong et al. additionally introduce a general, a location-based and a dot-product score function for the global attention mechanism as described in table [8.1](#tab:luong-score-functions).
| Score-function | Name |
|---|---|
...
Table [8.1](#tab:luong-score-functions): (#tab:luong-score-functions) Different score function proposed by Luong et al.
As Luong, Pham, and Manning ([2015](#ref-luong2015effective)) don’t use a bidirectional encoder, they simplify the hidden
...
approach. While it was used by Xu et al. ([2015](#ref-xu2015show)) for caption generation of images with a CNN
and by Gregor et al. ([2015](#ref-gregor2015draw)) for the generation of images, the first application and differentiable version for NMT
is from Luong, Pham, and Manning ([2015](#ref-luong2015effective)).
...
the global model which can be seen in Figure [8.2](attention-and-self-attention-for-nlp.html#fig:attention-plots-luong).
...
A Gaussian distribution centred around \(p_t\) is placed by Luong, Pham, and Manning ([2015](#ref-luong2015effective)) to favour alignment points closer to \(p_t\).
...
gradients for \(W_p\) and \(v_p\) and is thus ``differentiable almost everywhere’’ Luong, Pham, and Manning ([2015](#ref-luong2015effective)) while being less computationally expensive than global attention.
...
concepts can be found in Figure [8.3](attention-and-self-attention-for-nlp.html#fig:attention-mechanisms).
8.1.4 Attention Models
Attention was in the beginning not directly developed for RNNs or even NLP.
Attention was first introduced in Graves, Wayne, and Danihelka ([2014](#ref-GravesWD14)) with a content-based attention mechanism
...
Luong, Pham, and Manning ([2015](#ref-luong2015effective)), Bahdanau, Cho, and Bengio ([2014](#ref-bahdanau2014neural)) and Xu et al. ([2015](#ref-xu2015show)).
Xu et al. ([2015](#ref-xu2015show)) were the first publication to differentiate between soft/global and hard/local attention mechanisms and did this in the
context of Neural Image Caption with both mechanisms being close to what was used
by Luong et al. in the previous section. Cheng, Dong, and Lapata ([2016](#ref-cheng2016long)) were the first to introduce the concept of self-attention, the third big
category of attention mechanisms.
8.2 Self-Attention
Cheng, Dong, and Lapata ([2016](#ref-cheng2016long)) implement self-attention with a modified LSTM unit, the Long Short-Term
...
span is reached’’ Cheng, Dong, and Lapata ([2016](#ref-cheng2016long)).
Self-Attention, as the name implies, allows an encoder to attend to other parts of the input during processing as seen in Figure [8.4](attention-and-self-attention-for-nlp.html#fig:self-attention-cheng).
While the LSTMN introduced self-attention, it retains the drawbacks that come from
the use of a RNN which are discussed at the end of the Transformer section. Vaswani et al. ([2017](#ref-vaswani2017attention)) propose the Transformer architecture which uses
...
constraints that recurrent networks have. Models like ByteNet from Kalchbrenner, Espeholt, Simonyan, Oord, et al. ([2016](#ref-kalchbrenner2016neural)[a](#ref-kalchbrenner2016neural))
and ConvS2S from Gehring et al. ([2017](#ref-gehring2017convolutional)) alleviate the problem with sequential models
...
in number of operations needed. The Transformer architecture from Vaswani et al. ([2017](#ref-vaswani2017attention))
...
from attending to subsequent positions’’ Vaswani et al. ([2017](#ref-vaswani2017attention)). This, coupled with the
...
As seen in [8.7](attention-and-self-attention-for-nlp.html#fig:transformer-full), the Transformer uses positional encodings
added to the embeddings so the model can make use of the order of the sequence.
Vaswani et al. ([2017](#ref-vaswani2017attention)) use the sine and cosine function of different frequencies:
\[
...
\]
For further reasoning why these functions were chosen see Vaswani et al. ([2017](#ref-vaswani2017attention)).
8.2.1.1 The self-attention mechanism(s)
Vaswani et al. ([2017](#ref-vaswani2017attention)) describe attention functions as “mapping a query and a set of key-value pairs to an output,
where the query, keys, values, and output are all vectors. The output is computed as a weighted sum
of the values, where the weight assigned to each value is computed by a compatibility function of the
query with the corresponding key”.
The Query and the Key-Value pairs are used in the
...
general input to the encoder/decoder with different weight matrices as can be seen in figure [8.8](attention-and-self-attention-for-nlp.html#fig:self-attention-inputs).
...
As seen in Figures [8.5](attention-and-self-attention-for-nlp.html#fig:encoder-transformer), [8.6](attention-and-self-attention-for-nlp.html#fig:decoder-transformer)
and [8.7](attention-and-self-attention-for-nlp.html#fig:transformer-full), the Transformer uses an attention mechanism called
...
seen in [8.9](attention-and-self-attention-for-nlp.html#fig:multi-head-attention). This multi-dimensionality allows the attention
...
Vaswani et al. ([2017](#ref-vaswani2017attention)) use \(h = 8\) and \(d_k = d_v = d_{model}/h = 64\). This reduced dimensionality
...
The optional Mask-function seen in Fig. [8.10](attention-and-self-attention-for-nlp.html#fig:scaled-dot-prod-attention) is
...
computed as:
\[ \text{Attention}(Q,K,V) = \text{softmax}(\frac{QK^\top}{\sqrt{d_k}})V \] where \(\text{Attention}(Q,K,V)\) corresponds to an non-projected head of multi-head attention. These attention mechanisms allow Transformers to learn different and distant dependencies in language and are thus a good candidate for transfer learning.
...
input length \(n\) due to extensive parallelization as can be seen in Table [8.2](#tab:complexity-operations).
...
They are also a popular research topic as they are quadratically scaling, with regard to input length, complexity per layer inhibits their use for very long sequences and makes them time consuming to train. This quadratic complexity comes from the self-attention mechanism \(\text{Attention}(Q,K,V) = \text{softmax}(\frac{QK^\top}{\sqrt{d_k}})V\). The used softmax needs to calculate the attention score between the currently processed input \(x_i\) and every other input \(x_j\) for each \(j \in {1, \dots ,n}\) and \(i \in {1, \dots ,n}\). This limits the length of the context that Transformers can process and increases the time they need for training in practical uses. The computational costs are especially severe in very long tasks.
In the last few years new variations of the vanilla transformer, as described previously in the chapter, were published
which lower the computational cost (e.g. Shen et al. ([2018](#ref-shen2018efficient)) and Choromanski et al. ([2020](#ref-choromanski2020masked))).
Some of these variations of the Transformer architecture managed to decrease the complexity
from quadratic to \(O(N\sqrt{N})\) (Child et al. ([2019](#ref-child2019generating))), \(O(N\log N)\) (Kitaev, Kaiser, and Levskaya ([2020](#ref-kitaev2020reformer))) or \(O(n)\) (Wang et al. ([2020](#ref-wang2020linformer))) as seen in Table [8.2](#tab:complexity-operations).
...
Table [8.2](#tab:complexity-operations) : (#tab:complexity-operations) Complexity per layer and the number of sequential operations. Sparse Transformers are from Child et al. ([2019](#ref-child2019generating)), Reformer from Kitaev, Kaiser, and Levskaya ([2020](#ref-kitaev2020reformer)), Linformer from Wang et al. ([2020](#ref-wang2020linformer)) and Linear Transformers from Katharopoulos et al. ([2020](#ref-katharopoulos2020transformers)). \(n\) is the sequence length, \(k\) the kernel size, \(d\) the representation dimension and \(r\) the size of the neighbourhood in restricted self-attention. Source: Vaswani et al. ([2017](#ref-vaswani2017attention)) (Table 1) and Wang et al. ([2020](#ref-wang2020linformer)) (Table 1)
8.2.2 Transformers as RNNs
A version from Katharopoulos et al. ([2020](#ref-katharopoulos2020transformers)) uses a linear attention mechanism for
...
function, including kernels, as similarity function to generate a new attention mechanism Katharopoulos et al. ([2020](#ref-katharopoulos2020transformers)).
...
row of the query (Katharopoulos et al. ([2020](#ref-katharopoulos2020transformers))).
...
the exponential kernel Tsai et al. ([2019](#ref-tsai2019transformer)). This results in a computational cost of
\(O(ND^2M)\) which is favourable if \(N > D^2\). Katharopoulos et al. ([2020](#ref-katharopoulos2020transformers)) suggest \(\phi(x) = \text{elu}(x) + 1\) for \(N < D^2\).
For autoregressive tasks we want an attention mechanism that can’t look ahead of its position.
Vaswani et al. ([2017](#ref-vaswani2017attention)) introduced masked-self-attention for their decoder, which is adapted for linearised attention.
...
the feature map Katharopoulos et al. ([2020](#ref-katharopoulos2020transformers)).
Given the previous formalization of feature maps to replace the softmax we can rewrite the layers of a Transformers as a RNN. This RNN has two hidden states, the attention state \(s\) and the normalizer state \(z\) with subscripts denoting the timestep in recurrence. With \(s_0 = 0\) and \(z_0 = 0\) we can define \(s_i\) as \(s_i s_{i-1} + \phi(x_iW_K)(x_iW_V)^\top\) and \(z_i\) as \(z_i = z_{i-1} + \phi(x_iW_K)\) with \(x_i\) as the \(i\)-th input for the layer. The \(i\)-th output \(y_i\) can then be written as \(y_i = f_l(\frac{\phi(x_iW_Q)^\top s_i}{\phi(x_iW_Q)^\top z_i} + x_i)\) where \(f_l\) is the function given by the feed-forward network of a Transformer layer.
This shows that the Transformer layers can be rewritten into RNN layers, for all similarity functions
that can be represented with \(\phi\) (Katharopoulos et al. ([2020](#ref-katharopoulos2020transformers))), which are the first models
that used attention for NLP tasks.
References
Alammar, Jay. 2018. “The Illustrated Transformer [Blog Post].” Http://Jalammar.github.io/. [http://jalammar.github.io/illustrated-transformer/](http://jalammar.github.io/illustrated-transformer/).
...
Graves, Alex, Greg Wayne, and Ivo Danihelka. 2014. “Neural Turing Machines.” CoRR abs/1410.5401. [http://arxiv.org/abs/1410.5401](http://arxiv.org/abs/1410.5401).
...
Weng, Lilian. 2018. “Attention? Attention!” Lilianweng.github.io/Lil-Log. [http://lilianweng.github.io/lil-log/2018/06/24/attention-attention.html](http://lilianweng.github.io/lil-log/2018/06/24/attention-attention.html).
