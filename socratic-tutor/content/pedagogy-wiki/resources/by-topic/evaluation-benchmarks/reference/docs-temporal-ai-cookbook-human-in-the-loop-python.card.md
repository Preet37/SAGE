...
In a realistic scenario, a set of tools will have been provided to the LLM and the result might be a recommended tool call. In this case, if approved, the agent would invoke the tool via an activity. See the agentic loop with tool calling for guidance on how to use dynamic activities, allowing the tools to be loosely coupled from the agent implementation.
