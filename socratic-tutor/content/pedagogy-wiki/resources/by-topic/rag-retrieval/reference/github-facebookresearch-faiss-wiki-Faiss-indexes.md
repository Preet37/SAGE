...
-
[Notifications](/login?return_to=%2Ffacebookresearch%2Ffaiss)You must be signed in to change notification settings -
[Fork 4.3k](/login?return_to=%2Ffacebookresearch%2Ffaiss)
Faiss indexes
[27 revisions](/facebookresearch/faiss/wiki/Faiss-indexes/_history)
...
, a ResidualCoarseQuantizer
(see "Indexes based on a residual quantizer" section below) or ` a HNSW based quantizer.
...
The most popular cell-probe method is probably the original Locality Sensitive Hashing method referred to as [E2LSH] ([http://www.mit.edu/~andoni/LSH/](http://www.mit.edu/~andoni/LSH/)). However this method and its derivatives suffer from two drawbacks:
...
The beam size is set via the
max_beam_size
field of the ResidualQuantizer object: the larger the slower and the more accurate.The ResidualQuantizer object takes in its constructor the number of bits of each quantizer step. For example [16, 8, 8] means that there is one 16-bit quantizer (65536 centroids) followed by two 8-bit quantizers.
...
[CPU Faiss + Intel SVS - Overview](CPU-Faiss---Intel-SVS-%E2%80%90-Overview)
